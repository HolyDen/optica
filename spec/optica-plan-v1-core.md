...
**Keeping current:** Dependabot for automated update PRs; a **CI version matrix** testing the minimum *and* maximum declared versions of what it installs (not just latest — testing only latest can never catch use of an API absent in the lower bound); and changelog monitoring for the major dependencies (torch, FastAPI, open-clip-torch), since semantic breaks that don't trip a version constraint won't be caught automatically. The matrix verifies the declared range of **what each leg installs** — Core plus `optica[test]`, under the `>=min, <next_major` bounds `pyproject.toml` sets — and its cost is two legs on a suite built to be fast. **Two things it does not verify, and is not claimed to.** The torch stack: those four packages are deliberately unpinned, so there is no minimum to test against, and their compatibility is asserted by the pairing rule and checked at install time instead. The extras groups: `FastAPI`, `uvicorn` and `open-clip-torch` are declared and carry bounds, but no leg installs them — and `open-clip-torch` **cannot** be added, since it declares torch, torchvision and timm unconditionally and would drag the torch stack into CI. Adding `FastAPI` and `uvicorn` is a cost choice rather than a constraint, deliberately not made here: installing a package the suite does not exercise verifies that its range resolves, not that its API works.
...
{ "version": 1, "created": "…", "updated": "…",
  "deselected": { "cat": ["…IMG_0007.jpg"], "dog": [] },
  "active_class": "cat" }
```
...
**"Start fresh" deletes the staging and any state file that accompanies it.** For an interrupted fetch there is no state file — the `.partial` directory is itself the state, and deleting it is the whole operation.
...
- **Open Images via direct GCS access.** No extra dependency (reuses httpx); label mapping cached and verified on load.
...
- **Curation server is localhost only** — binds to `127.0.0.1`, never network-exposed, shuts down after use.
